...
export default function InfocyleLandingPage() {
  ...
  const aiSessionPromiseRef = useRef(null);
  ...
  const toggleLiveSession = async () => {
    if (isLiveActive) {
      if (aiSessionPromiseRef.current) {
         aiSessionPromiseRef.current.then(session => session.close());
    ...
    try {
      // Use the installed package instead of the CDN
        ...
        callbacks: {
          onopen: () => {
            ...
            recorder.port.onmessage = (event) => {
              ...
              if (aiSessionPromiseRef.current) {
                aiSessionPromiseRef.current.then(session => {
  ...
  return (
    <div className="bg-white text-slate-600 font-sans antialiased overflow-x-hidden min-h-screen selection:bg-teal-500 selection:text-white pb-24">
      ...
      {/* Floating Gemini AI Voice Agent Widget */}
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
        {isLiveActive && (
          <div className="bg-white border border-slate-200 p-4 rounded-2xl mb-4 max-w-xs shadow-2xl animate-in slide-in-from-bottom-2">
            <div className="flex items-center gap-2 mb-2 border-b border-slate-100 pb-2">
              <Bot className="w-4 h-4 text-teal-600" />
              <span className="text-xs font-bold text-teal-600 uppercase tracking-widest">Infocyle AI Rep</span>
            </div>
            <p className="text-sm text-slate-600 font-medium italic">"{liveTranscript}"</p>
          </div>
        )}
        <button
          onClick={toggleLiveSession}
          disabled={isConnecting}
          className={`flex items-center justify-center gap-2 px-6 py-3 rounded-full font-bold shadow-lg transition-all ${
            isLiveActive 
              ? 'bg-red-50 text-red-600 border border-red-200 hover:bg-red-100' 
              : 'bg-teal-500 text-white shadow-teal-500/30 hover:bg-teal-400'
          } ${isConnecting ? 'opacity-50 cursor-not-allowed' : 'hover:-translate-y-1'}`}
        >
          {isConnecting ? (
            <span className="flex items-center gap-2"><span className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full"></span> Connecting...</span>
          ) : isLiveActive ? (
            <><MicOff className="w-5 h-5" /> End Conversation</>
          ) : (
             <><Mic className="w-5 h-5" /> Talk to AI Agent</>
          )}
        </button>
      </div>
